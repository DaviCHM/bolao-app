...
Bolão parimutuel entre amigos: sem casa de apostas e sem odds fixas. O pool inteiro é
...
um mercado com uma pergunta de duas opções ("o time X vence?"), os participantes apostam
num dos lados e o criador resolve o resultado quando o evento termina. Não existe casa
cobrando margem: tudo que foi apostado forma um pool, e o pool é dividido
proporcionalmente entre quem apostou no lado vencedor. O acerto em dinheiro de verdade
acontece fora do app, na confiança entre os amigos. O sistema registra saldos, apostas e
o rateio.

Exemplo: R$ 150 apostados em cada lado formam um pool de R$ 300. Cada vencedor recebe o
dobro do que apostou, porque `payout = valor_apostado / total_vencedor * pool`. Quem
apostou R$ 30 recebe R$ 60; quem apostou R$ 10 recebe R$ 20.
...
- O settlement parimutuel usa o método *largest remainder* para distribuir o resíduo de
  centavos, e a invariante `soma(payouts) == pool` é testada. O `SettlementCalculator` é
  uma função pura, sem Spring nem banco.
- Concorrência tratada em duas camadas: lock pessimista (`PESSIMISTIC_WRITE`) no `Market`
  serializa apostas e resolução, e lock otimista (`@Version`) no `User` cobre apostas
  simultâneas em mercados diferentes. Conflito retorna `409`.
- Multi-tenancy por grupo: sessão via header `Authorization: Bearer <token>` com TTL
  ...
  em vez de `403`, para não vazar que o recurso existe.
...
2. Suba os serviços:
   ...
   `docker compose down` (somem apenas com `down -v`).
...
| --- | --- | --- |
| POST | `/auth/register` | Cria um grupo `{nome, senha}` e abre sessão, retornando `{token, grupoId, grupoNome}` |
...
Rodam em H2 em memória, sem precisar de MySQL. São 38 testes cobrindo:
